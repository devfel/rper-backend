...
@injectable()
export class DeleteRperEditResourceService {
  constructor(
  ...
  async execute(data: ICreateRperEditResourceDTO): Promise<void> {
    ...
    if (!existingEditResource) {
      throw new AppError('Editing resource not found!', 404);
    }

    await this.rperEditResourceRepository.delete(existingEditResource.id);
  }
}
